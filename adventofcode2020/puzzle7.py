...
def fn(rules):
    graph = {}
    for rule in rules:
        rule = re.sub(" bags", "", rule)
        rule = re.sub(" bag", "", rule)
        nodes = rule.split("contain")
        start = nodes[0].strip()
        ends = nodes[1].split(",")
        ...
        for str in ends:
            ...
            str = str.strip()
            pruned_ends.append(str)

        graph[start] = pruned_ends
    ...
    for bag in graph.keys():
        ...
        path = find_path(graph, bag, "shiny gold")
        if path != [] and path != None:
            possible_bags.append(bag)

    print(possible_bags)
    print(possible_bags.__len__())
    return possible_bags.__len__()
    

#fn(example)
#fn(my)

def find_bags(graph, start, curr_bags=0):
    if(graph[start] == ["no other"]):
        return curr_bags
    
    for bag in graph[start]:
        curr_bags += find_bags(graph, bag, 1)
        ...
        for str in ends:
            ...
            if str == "no other":
                ...
                for i in range(0, int(items[0])):
                    pruned_ends.append(items[1])


        graph[start] = pruned_ends

    answer = find_bags(graph, "shiny gold")
    print(answer)
    return answer
