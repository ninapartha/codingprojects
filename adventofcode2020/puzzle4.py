...
wanted = ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]
...
def fn2(pwds):
    ...
    for pwd in pwds:
        ...
        is_valid = True
        for str in wanted:
            if not entries.__contains__(str):
                is_valid = False
                continue
            val = entries[str]
            if (str == 'byr'):
                ...
                h = val[:-2]
                unit = val[val.__len__() - 2:]
                if unit == 'cm' and (int(h) < 150 or int(h) > 193):
                    is_valid = False
                    continue
                elif unit == 'in' and (int(h) < 59 or int(h) > 76):
                    is_valid = False
                    continue
            elif (str == 'hcl'):
                if val[0] != '#' or val.__len__() != 7:
                    is_valid = False
                    continue
                pattern = re.compile("[0-9a-f]+")
                result = re.sub(pattern, '', val[1:])
                if result != '':
                    is_valid = False
                    continue
            elif (str == 'ecl'):
